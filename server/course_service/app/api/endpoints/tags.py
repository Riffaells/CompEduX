...
async def get_tags(
    ...
    if search:
        return tag_crud.search_by_name(db, search, skip=skip, limit=limit)
...
async def create_tag(
    ...
    existing_tag = tag_crud.get_by_name(db, tag.name.get("en", ""))
    if existing_tag:
    ...
    return tag_crud.create(db, obj_in=tag)
...
async def update_tag(
    ...
    existing_tag = tag_crud.get(db, tag_id)
    if not existing_tag:
        logger.warning(f"Tag with ID {tag_id} not found")
        raise HTTPException(
            status_code=status.HTTP_404_NOT_FOUND,
    ...
    if tag.name and "en" in tag.name:
        duplicate = tag_crud.get_by_name(db, tag.name["en"])
    ...
    # Update the tag
    updated_tag = tag_crud.update(db, db_obj=existing_tag, obj_in=tag)
...
async def delete_tag(
    ...
    if tag_crud.is_tag_in_use(db, tag_id):
...
async def get_tag_by_name(
    ...
    tag = tag_crud.get_by_name(db, name)
    if not tag:
